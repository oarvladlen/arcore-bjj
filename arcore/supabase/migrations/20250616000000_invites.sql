-- mirrors schema-invites.sql
-- Arcore invitations + phone/marketing
...
returns json language sql stable security definer set search_path = public as $$
  select json_build_object(
    ...
    'email', i.email, 'phone', i.phone, 'name', m.name,
    'member_id', i.member_id, 'status', i.status
  )
  from invitations i join members m on m.id = i.member_id
...
begin
  ...
  where token = p_token and status = 'pending' and expires_at > now() limit 1;
  if not found then return json_build_object('ok', false, 'error', 'invalid_invite'); end if;
  ...
  update profiles set member_id = v_inv.member_id, role = 'member',
    phone = coalesce(phone, v_inv.phone), email = v_inv.email where id = auth.uid();
  update members set status = 'experimental', email = v_inv.email,
    phone = coalesce(members.phone, v_inv.phone) where id = v_inv.member_id;
